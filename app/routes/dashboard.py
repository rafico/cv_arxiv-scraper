...
@dashboard_bp.route("/papers/<int:paper_id>/pdf")
def paper_pdf(paper_id: int):
    """Serve the paper's PDF, downloading and caching it on first request.

    Cache key is the numeric paper id, not the arxiv id — old-style arXiv ids
    contain "/" and would escape the cache directory.
    # ponytail: no cache eviction; add one if instance/pdfs ever grows painful.
    """
    import os
    import uuid

    from app.services.thumbnail_generator import _download_pdf

    paper = Paper.query.get_or_404(paper_id)
    if not paper.pdf_link:
        return ("", 404)

    pdf_dir = Path(current_app.instance_path) / "pdfs"
    pdf_path = pdf_dir / f"{paper.id}.pdf"
    if not pdf_path.exists():
        try:
            content = _download_pdf(paper.pdf_link)
        except Exception:
            return ("Could not fetch the PDF from arXiv", 502)
...
@dashboard_bp.route("/papers/<int:paper_id>/read")
def paper_reader(paper_id: int):
    paper = Paper.query.get_or_404(paper_id)
    return render_template("reader.html", paper=paper, csrf_token=get_or_create_csrf_token())
